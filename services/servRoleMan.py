...
class ServRoleMan( object ):
	...
	def run( self ):
		...
		print('RUN ROLEMANNNNN____________________________________  {}'.format( self.env['QUERY_STRING'] ))				
		...
		if( qStr['manRole'].strip().lower() == 'add' ):
			#print('\n\n\n\nINTRO IF ADDDD ROOLE')
			dataMan.addRole( RoleProps('plchldr', qStr['priority'], qStr['rolename'], 'plchldr' ) ) 
		elif( qStr['manRole'].strip().lower() == 'remove' ):
			''' TODO - CODING METHOD DELROLE AND DISABLE ROLE '''
			dataMan.delRole( RoleProps('plchldr', qStr['priority'], qStr['rolename'], 'plchldr' ) ) 
